...
#include "C:\Users\Utilisateur\Downloads\metis-5.1.0\include\metis.h"
...
std::vector<idx_t> func(std::vector<idx_t> xadj, std::vector<idx_t> adjncy, std::vector<idx_t> weights,idx_t nParts) {
    idx_t nVertices = xadj.size() - 1; // Number of nodes
    idx_t nWeights = 1;
    idx_t objval;
    std::vector<idx_t> part(nVertices, 0);

    int ret = METIS_PartGraphKway(&nVertices, &nWeights, xadj.data(), adjncy.data(), NULL, NULL,
        weights.data(),  &nParts, NULL,
...
std::vector<Graph> GraphPartitioning(Graph G, int proc, int s) {
    std::vector<idx_t> weights(0);
    std::vector<idx_t> xadj(0);
    std::vector<idx_t> adjncy(0);
    idx_t nbp = proc;
    ...
    std::vector<idx_t> part=func(xadj,adjncy,weights,proc);
    std::cout << part[20] << std::endl;
...
void ParDijk(Graph G, std::vector<std::vector<Node*> > Messagearray, std::vector<int> sendTag, std::vector<std::pair<Node*, int> >& result) {
    ...
    while (true) {
        ...
        //exitaccumulate(sendTag, &ExitFlag);
        if (ExitFlag) {
...
int main() {
    std::string filename = "256v2048e.txt";
    ...
    //printSolution(Q1);
    //std::vector<Graph> graphs = GraphPartitioning1(G1, 100, 0);
    const std::chrono::time_point<std::chrono::steady_clock> start1 = std::chrono::steady_clock::now();
    //std::vector<Graph> graphs = GraphPartitioning1(G1, 10, 0);
    std::vector<std::pair<Node*, int> > path = ParallelSSSP(G1, 50);
